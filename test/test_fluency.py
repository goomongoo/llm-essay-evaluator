...
from config import DATASET_DIR, FLUENCY_CRITERIA
...
def print_and_evaluate_total_content_scores(limit: int = 10, randomize: bool = True):
    ...
    for idx, path in enumerate(file_paths):
        ...
        gt_scores = data["score"]["essay_scoreT_detail"]["essay_scoreT_exp"]
        llm_result = evaluate_fluency({
            "prompt": prompt,
            "input_text": llm_input
        })

        human_total = round(sum(gt_scores), 2)
        llm_total = round(sum(llm_result[criterion] for criterion in FLUENCY_CRITERIA), 2)
