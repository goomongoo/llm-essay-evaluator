...
import tests.prompts.coherence_prompt as coherence_prompt
...
def evaluate_coherence_test(context: dict):
    ...
    for i in range(repeat_count):
        logger.info(f"[Coherence] LLM Evaluation iteration {i + 1}/{repeat_count}")
        ...
        try:
            ...
            logger.warning(f"[Coherence] Failed on iteration {i + 1}: {e}")
...
def main():
    ...
    for path in tqdm(file_paths, desc="Evaluating essays"):
        ...
        human_score = float(data["score"]["essay_scoreT_detail"]["essay_scoreT_org"])
        llm_score = evaluate_coherence_test({
            "prompt": prompt,
    ...
    print("\nCorrelation Results (Coherence Score)")
